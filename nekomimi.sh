...
case "$command" in
	-ua)
		...
		while read LINE; do
			let i++
			echo -e ">>>\E[35mОбработка тега ($i/$total): \E[37m$LINE"
		...
		rm -f tags.txt
		echo -e ">>>\E[35mОбновление тегов завершено\E[37m"
		if [ -e "$pref.NewPostsCount.txt" ]; then
			echo ">>>"
			echo -e ">>>\E[35mНовые посты у тегов:\E[37m"
			cat "$pref.NewPostsCount.txt"
			echo -e ">>>\E[35mОткрыть папки с новыми тегами?\E[37m(y/n)"
			read ans
			if [ "$ans" == "y" ]; then
	...
	-gl)
		if [ -e "$tag_id" ]; then
			...
			while read LINE; do
				...
				echo -e ">>>\E[35mОбработка тега ($i/$total): \E[37m$LINE"
				$scriptfolder/getinfo.sh "$LINE" "$pref" "$api_url" "$pref_dl" "$page" "$bl_tags"
				$scriptfolder/dloader.sh "$LINE" "$pref" "true"
				echo ">>>"
			done < $tag_id
			echo -e ">>>\E[35mСкачивание тегов завершено\E[37m"
		fi
		;;
	-u)
		if [ -d "$tag_id" ];then
			...
			echo -e ">>>\E[35mОбработка тега: \E[37m$tag_id"
			...
			echo -e ">>>\E[35mОбновление тега завершено\E[37m"
			if [ -e "$pref.NewPostsCount.txt" ]; then
				echo ">>>"
				...
				echo -e ">>>\E[31mТег \E[37m'$tag_id' \E[31mне найден!\E[37m"
		fi
		;;
	-n)
		rm -f "$pref.NewPostsCount.txt"
		...
		echo -e ">>>\E[35mСкачивание тега завершено\E[37m"
		if [ -e "$pref.NewPostsCount.txt" ]; then
		echo ">>>"
		echo -e ">>>\E[35mОткрыть папку с новыми постами?\E[37m(y/n)"
		read ans
	...
	--help)
		...
		;;
	*)
		echo "Для вывода справки попробуйте nekomimi.sh --help"
esac
