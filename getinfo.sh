...
res=`wget "$api_url&tags=$tag$bl_tags&limit=1" --no-check-certificate -q -U "$uag" -O -`
...
for ((i=0; i<=$pcount; i++)) do wget "$api_url&tags=$tag$bl_tags&limit=1000&$page=$i" --no-check-certificate -q -U "$uag" -O - |grep -E -o -e 'file_url=[^ ]+' -e ' id=[^ ]+'|sed -e 's/file_url=//g' -e 's/id=//g' -e 's/\"//g' >>UrlsIds.txt
sed -i ':a;N;$!ba;s/\n / /g' UrlsIds.txt 
done;
