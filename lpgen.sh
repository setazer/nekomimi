...
if [ -a "lastpost.txt" ];then
	...
	if [ ! -z $lastpost ];then
		echo -e ">>>\E[32m‘айл последнего поста найден\E[37m"
		echo -e ">>>\E[36mID последнего поста: \E[37m$lastpost"
		exit
	fi
fi
lastpost=0
echo -e ">>>\E[31mID последнего поста не найден. √енераци¤ по имеющимс¤ пикчам\E[37m"
...
rm files.txt
echo -e ">>>\E[32mќпределен ID последнего: \E[37m$lastpost"
